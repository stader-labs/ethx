...
export async function distributeKeys(keystore: any, keystorePassword: any, operators: any, operatorIndex: any) {
  const operatorIds = []
  const operatorKeys = []
  ...
  for (const operator of operators[operatorIndex]) {
    ...
    operatorsPublicKeys.push(web3.eth.abi.encodeParameter('string', encode(operator.operatorKey)))
  }
