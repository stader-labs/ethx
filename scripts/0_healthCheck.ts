...
async function main() {
  ...
  const ssvPoolBalance = await provider.getBalance(staderSSVPoolInstance.address)

  const ssvPoolAvailableValidators = Math.floor(parseInt(ssvPoolBalance) / (32 * 10 ** 18))
  ...
  const registered = await startWorkflow(KEYSTORE_PASSWORD, OPERATOR_INDEX, ssvPoolAvailableValidators)
