...
contract StaderOracle is IStaderOracle, AccessControlUpgradeable{
    ...
    function submitBalances(uint256 _block, uint256 _totalEth, uint256 _stakingEth, uint256 _ethxSupply, uint256 _protocolFee) override external {
        ...
        // Get submission keys
        bytes32 nodeSubmissionKey = keccak256(abi.encodePacked(msg.sender, _block, _totalEth, _stakingEth, _ethxSupply, _protocolFee));
        bytes32 submissionCountKey = keccak256(abi.encodePacked(_block, _totalEth, _stakingEth, _ethxSupply, _protocolFee));
