...
contract PermissionedNodeRegistryMock {
    ...
    function allocateValidatorsAndUpdateOperatorId(
        uint256
    ) external pure returns (uint256[] memory selectedOperatorCapacity) {
    ...
    function getOperatorTotalNonTerminalKeys(address, uint256, uint256) public pure returns (uint64) {
        return 5;
    }
    ...
    function validatorIdByPubkey(bytes memory) external pure returns (uint256) {
        return 1;
    }
